...
| --- | --- | --- | --- |
| **0** | **Fundación** | Crear proyecto Supabase. Definir esquema de BD inicial (solo `users`, `games`, `user_games`). El resto de tablas (grupos, logros) se migrarán en sus fases. Proyecto Flutter vacío conectado a Supabase. Autenticación. | Login funcional. Schema base creado. La app arranca y conecta con el backend. |
