...
class _IntegrationsScreenState extends State<IntegrationsScreen> {
  ...
  Future<void> _linkSteam() async {
    ...
    try {
      ...
      if (data != null && data['success'] == true) {
        ...
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cuenta de Steam vinculada con éxito.')));
      }
    } on FunctionException catch (e) {
       if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: ${e.details ?? e.toString()}')));
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error al vincular: $e')));
    } finally {
  ...
  Future<void> _unlinkSteam() async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cuenta desvinculada.')));
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error al desvincular: $e')));
    } finally {
      if (mounted) setState(() => _isLinkingSteam = false);
    }
  }
            ...
            Container(
              ...
              child: _steamId == null || _steamId!.isEmpty
                  ? Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Vincula tu cuenta de Steam para importar tus juegos y tiempo de juego. Pega tu URL de perfil de Steam (ej. https://steamcommunity.com/id/tunombre) o tu SteamID64.', style: TextStyle(fontSize: 13, color: Colors.grey)),
                        ...
                        Row(
                          children: [
                            ...
                            ElevatedButton(
                              onPressed: _isLinkingSteam ? null : _linkSteam,
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            TextButton(
                              onPressed: _isLinkingSteam ? null : _unlinkSteam,
                        ...
                        SizedBox(
                          ...
                          child: OutlinedButton.icon(
                            ...
                            label: const Text('Importar biblioteca de Steam'),
                            onPressed: () {
                              showModalBottomSheet(
                                context: context,
                                isScrollControlled: true,
                                backgroundColor: Colors.transparent,
                                builder: (_) => const SteamImportSetupScreen(),
                              );
                            },
                          ),
                        ),
