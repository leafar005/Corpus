...
class SteamImportProgressScreen extends StatefulWidget {
  ...
  const SteamImportProgressScreen({super.key, required this.minPlaytimeMinutes});

  @override
  State<SteamImportProgressScreen> createState() => _SteamImportProgressScreenState();
}
...
class _SteamImportProgressScreenState extends State<SteamImportProgressScreen> {
              ...
              if (mounted) {
                setState(() {
                  ...
                  if (status == 'failed') {
                    ...
                    _status = 'Procesando $_processedGames de $_totalGames juegos...';
                  }
                });
              }
            }
      ...
      try {
        ...
        if (data != null) {
          ...
          for (final ug in userList) {
            ...
            final cg = catalogList.firstWhere((c) => c['igdb_id'] == gameId, orElse: () => null);
            if (cg != null) {
              _accumulatedRows.add(CsvGameRow(
          ...
          if (data['done'] == true) {
            ...
            if (mounted) {
              ...
              Navigator.pushReplacement(
                context,
                MaterialPageRoute(
                  builder: (context) => ImportPreviewScreen(rows: _accumulatedRows),
                ),
              );
            }
          } else {
          ...
          if (mounted) setState(() { _isError = true; _status = 'Respuesta vacía'; });
        }
        ...
        if (mounted) {
          setState(() {
            ...
            if (msg.contains('PRIVATE_PROFILE')) {
              _status = 'Tu perfil de Steam es privado. Por favor, haz pública tu biblioteca en Steam y vuelve a intentarlo.';
            } else {
  ...
  Widget build(BuildContext context) {
    final progress = _totalGames > 0 ? (_processedGames / _totalGames).clamp(0.0, 1.0) : 0.0;

    return Scaffold(
      appBar: AppBar(
        title: const Text('Progreso de Importación'),
        automaticallyImplyLeading: _isDone || _isError, // Solo permitir salir si acabó o falló
      ),
      body: Center(
        child: Padding(
