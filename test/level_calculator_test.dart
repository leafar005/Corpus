...
void main() {
  group('LevelCalculator Gamification Logic', () {
    test(
      'getLevel calcula el nivel correctamente usando la fórmula polinomial',
      () {
    ...
    test(
      'getProgressFraction calcula la fracción de progreso en el nivel actual',
      () {
        expect(LevelCalculator.getProgressFraction(0), 0.0);
        expect(
          LevelCalculator.getProgressFraction(50),
          0.5,
        ); // 50 de 100 XP en Nivel 1
        expect(
          LevelCalculator.getProgressFraction(100),
          0.0,
        ); // Recién llegado al Nivel 2 (0 de 200 XP)
        expect(
          LevelCalculator.getProgressFraction(200),
          0.5,
        ); // 100 de 200 XP en Nivel 2
      },
    );
